...
/*0x18 address for v1 to use i2c, a3 for 0x10*/
#define I2CCHIP_ADDRESS_V1 0x1A //vi has the most 4 line to be output, so v1 has 4 slave address to define
...
#if defined(VATEK_V1)
static vatek_result hms_i2c_write_v1(Pvatek_i2c vi2c, uint8_t* pbuf, uint32_t len)
{
    vatek_result result = vatek_result_unknown;
    ...
    if ((result = vi2c->start(vi2c->hboard, I2CCHIP_WRADDR_V1, 0)) != vatek_result_success)
        return vatek_result_i2cfail;
...
static vatek_result hms_i2c_read_v1(Pvatek_i2c vi2c, uint8_t* pbuf, uint32_t len)
{
    vatek_result result = vatek_result_unknown;
    ...
    if ((result = vi2c->start(vi2c->hboard, I2CCHIP_RDADDR_V1, 0)) != vatek_result_success)
        return vatek_result_i2cfail;
...
{  /* for internal debug only */
    ...
    if ((result = hms_i2c_write_v1(handle->vi2c, &i2c_cmd[0], 5)) != vatek_result_success)
        return result;
    
    i2c_cmd[0] = 0;
    if ((result = hms_i2c_write_v1(handle->vi2c, &i2c_cmd[0], 1)) != vatek_result_success)
        return result;

    if ((result = hms_i2c_read_v1(handle->vi2c, &i2c_cmd[0], 4)) != vatek_result_success)
        return result;
...
vatek_result vatek_hms_write_reg_v1(Phms_handle handle, uint32_t addr, uint32_t val)
{
    ...
    if ((result = hms_i2c_write_v1(handle->vi2c, &i2c_cmd[0], 9)) != vatek_result_success)
        return result;
...
vatek_result vatek_hms_read_halbuf_v1(Phms_handle handle, uint32_t addr, uint8_t* pbuf, uint32_t len)
{
    ...
    {
        val = 0;
        result = vatek_hms_read_hal_v1(handle, addr, &val);
        pbuf[idx] = val & 0xFF;
        pbuf[idx+1] = (val >> 8) & 0xFF;
        pbuf[idx+2] = (val >> 16) & 0xFF;
        pbuf[idx+3] = (val >> 24) & 0xFF;
...
vatek_result vatek_hms_issystemidle_v1(Phms_handle handle)
{
    ...
    if ((result = vatek_hms_read_hal_v1(handle, HALREG_SYS_STATUS_0, &val)) == vatek_result_success)
    {
        if (val != SYS_STATUS_IDLE)
            return vatek_result_busy;

        if ((result = vatek_hms_read_hal_v1(handle, HALREG_BROADCAST_CNTL, &val)) == vatek_result_success)
        {
            if (val != 0)
                return vatek_result_badstatus;
        }
