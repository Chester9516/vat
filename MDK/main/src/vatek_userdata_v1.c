...
vatek_result vatek_userdata_setvideocodec_v1(video_encode_type type)
{
...
vatek_result vatek_userdata_raw_insert_v1(Phms_handle handle, uint16_t data_num, uint8_t *data)
{
...
vatek_result vatek_userdata_cc_insert_v1(Phms_handle handle, uint16_t cc_num, Puserdata_cc_parm cc)
{
    vatek_result result = vatek_result_unknown;
    uint32_t val = 0;
    uint8_t buf[ENC_EXTDATA_MAXLEN] = {0};
    ...
    if (vatek_hms_issystemrun_v1(handle) != vatek_result_success)
        return vatek_result_idle;  

    if ((result = vatek_hms_read_hal_v1(handle, HALREG_ENC_EXTDATA, &val)) != vatek_result_success)
        return result;
	...
	if ((result = vatek_hms_write_halbuf_v1(handle, HALREG_EN_EXTDATA_START, buf, len)) != vatek_result_success)
        return result;
    
    if ((result = vatek_hms_write_hal_v1(handle, HALREG_ENC_EXTDATA, cmd)) != vatek_result_success)
        return result;
