...
#ifndef _PHY_EP9555E_INTERNAL_
#define _PHY_EP9555E_INTERNAL_
    ...
    #define EP9555E_MANUAL_VFMT_CNTL			        0x2301
    ...
    #define EP9555E_AUDIO_SAMPLERATE_32KHZ              0
    #define EP9555E_AUDIO_SAMPLERATE_44_1KHZ            1
    #define EP9555E_AUDIO_SAMPLERATE_48KHZ              2
    
    #define EP9555E_SYNC_H_LEN                          8
    #define EP9555E_SYNC_V_LEN                          5
    #define EP9555E_COLOR_INIT                          0xFF

    #define EP9555E_ERR(fmt,...)                VATEK_ERR(ep9555e,fmt,##__VA_ARGS__)
    #define EP9555E_LOG(fmt,...)                VATEK_LOG(ep9555e,fmt,##__VA_ARGS__)
    
    #define EP9555E_CHIP_WR(i2c,reg,buf,len)    i2c_write(i2c,EP9555E_CHIP_ADDR,reg,buf,len)
    #define EP9555E_CHIP_RD(i2c,reg,buf,len)    i2c_read(i2c,EP9555E_CHIP_ADDR,reg,buf,len)
    
    #define EP9555E_SYS_WR(i2c,reg,buf,len)     i2c_write(i2c,EP9555E_SYS_ADDR,reg,buf,len)
    #define EP9555E_SYS_RD(i2c,reg,buf,len)     i2c_read(i2c,EP9555E_SYS_ADDR,reg,buf,len)
    
    #define EP9555E_ISP_WR(i2c,reg,buf,len)     i2c_write(i2c,EP9555E_ISP_ADDR,reg,buf,len)
    #define EP9555E_ISP_RD(i2c,reg,buf,len)     i2c_read(i2c,EP9555E_ISP_ADDR,reg,buf,len)

    typedef enum _ep9555e_outputmode
    {
    	ep9555e_interlaced_2_p = 1,     // auto setting output video
    	ep9555e_bypassmode = 0,         // auto setting output video
    	ep9555e_manual = 2,             // manual
    }ep9555e_outputmode;

    typedef enum _ep9555e_baseclock
    {
        baseclock_1000 = 0,
        baseclock_1001 = 1,
    }ep9555e_baseclock;

    typedef enum _ep9555e_status
    {
        epstatus_invalid = -2,
        epstatus_hw_fail = -1,
        epstatus_idle = 0,
    ...
    {
        Pvatek_i2c vi2c;
        ep9555e_outputmode outputmode;
        ep9555e_baseclock baseclk;
