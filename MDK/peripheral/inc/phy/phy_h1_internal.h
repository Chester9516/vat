...
#ifndef _PHY_H1_INTERNAL_
#define _PHY_H1_INTERNAL_
    ...
    #define H1_LOG(fmt,...)                     VATEK_LOG(h1,fmt,##__VA_ARGS__)
    
    #define H1_REG_WR(i2c,reg,val)              i2c_reg_write(i2c,reg,val)
    #define H1_REG_RD(i2c,reg,val)              i2c_read(i2c,H1_I2C_ADDR,reg,val,1)
    
    #define H1_WR_DREG(i2c,reg,buf)             i2c_write_double_reg(i2c,reg,buf)
    
    #define IS_SUCCESS(result)                  (result == vatek_result_success)
    #define IS_480I(resolution)                 (resolution==vi_resolution_480i60 || resolution==vi_resolution_480i59_94)

    typedef enum _h1_baseclk
    {
        h1_baseclk_1000 = 0,
        h1_baseclk_1001 = 1,
    }h1_baseclk;

    typedef enum _h1_status
    {
        h1_hw_fail        = 0,
        h1_invailed       = 1,
        h1_idle           = 2,
    ...
    typedef enum _h1_output_mode
    {
        ...
        h1_output_max           = h1_output_progressive,
    }h1_output_mode;

    typedef struct _h1_handle
    {
        Pvatek_i2c          vi2c;
        h1_output_mode output_mode;
        h1_baseclk     baseclk;
