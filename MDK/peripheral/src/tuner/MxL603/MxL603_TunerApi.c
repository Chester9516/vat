...
#include "tuner/MxL603/MxL603_TunerCfg.h"
#include "tuner/MxL603/MxL_Debug.h"

/* MxLWare Driver version for MxL603 */
const UINT8 MxLWare603DrvVersion[5] = {2, 1, 3, 5, 0}; 
                                           ...
                                           MXL603_CHAN_TUNE_CFG_T chanTuneCfg)
{
  ...
  if (chanTuneCfg.startTune == MXL_ENABLE)
  {
    if (chanTuneCfg.signalMode <= MXL603_DIG_J83B) 
    {
      ...
      if (freqLutPtr)
        status |= MxL603_Ctrl_SetRfFreqLutTblReg(devId, chanTuneCfg.freqInHz, freqLutPtr);
...
//****************  Register Test ***************************/
UINT8 Test_Tuner(UINT8 devId)
{

	UINT8 status = MXL_SUCCESS;
  	UINT8 readData = 0;	

	status = MxLWare603_OEM_WriteRegister(devId, 0x10, 0x11); 
  	status = MxLWare603_OEM_ReadRegister(devId, 0x10, &readData);

  	printf("Test_Tuner devId = %02x, addr=0x10, data=11:%02x\n", devId, readData);
