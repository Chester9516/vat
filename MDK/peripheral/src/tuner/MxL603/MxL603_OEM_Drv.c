...
MXL_STATUS MxLWare603_OEM_SetActiveI2C(void *pi2c)
{
    if (pi2c == NULL)
        return MXL_FAILED;
...
*/
    ...
    buffer[1] = RegData;
    size = 2;
    if (_I2C_Write(i2c_addr, buffer, &size) != vatek_result_success)
        status = MXL_FALSE;
...
*/
  ...
  /* If OEM data is not required, customer should treat devId as I2C slave Address */
    ...
    size = 2;
    result = _I2C_Write(i2c_addr,buffer,&size);
    size = 1;
    result |= _I2C_Read(i2c_addr,DataPtr,&size);
