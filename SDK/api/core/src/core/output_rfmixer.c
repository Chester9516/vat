...
vatek_result rfmixer_r2_add_tune_table(Pr2_tune_handle pr2tune, uint32_t freq, Pr2_tune_item* pitem)
{
    ...
    else if (pr2tune->items_num == 0 || pr2tune->items_num == 1)index = pr2tune->items_num;
    else if (pr2tune->rules[pr2tune->items_num - 1].freq < freq)index = pr2tune->items_num;
    else
    {
        int32_t i = 0;
...
vatek_result rfmixer_r2_del_tune_table(Pr2_tune_handle pr2tune, uint32_t freq)
{
    ...
    else
    {
		...
		{
			int32_t lne = sizeof(r2_tune_item) * (pr2tune->items_num - (index + 2));
