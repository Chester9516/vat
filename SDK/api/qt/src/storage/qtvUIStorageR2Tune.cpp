...
public:
	...
	QWidget* createEditor(QWidget* parent, const QStyleOptionViewItem& option, const QModelIndex& index) const override
	{
		...
		if (m_r2table->_mode() == qr2tune_advance)
		{
			...
			else if (index.column() == R2TABLE_IDX_PA)return new qi_r2tune_editor(R2_RANGE_POWER,(prule->pagain >> 4 & 0xF),parent);
		}
		else
		{
			if (index.column() == R2TABLE_IDX_NORM_GPIO)return new qi_r2gpio_editor(prule->gpiocntl, parent);
			else if (index.column() == R2TABLE_IDX_NORM_PA)return new qi_r2tune_editor(R2_RANGE_POWER, (prule->pagain >> 4 & 0xF), parent);
		}
		...
		if (m_r2table->_mode() == qr2tune_advance)
		{
			...
			else
			{
				...
				else if (index.column() == R2TABLE_IDX_PA)
				{
					...
					prule->pagain |= (pedit->value() & 0xF) << 4;
				}
			}
		}
		else
...
{
	rfmixer_r2_reset_tune_table(m_r2tune);
	...
	if (!path.isEmpty())
	{
		...
		if (frule.open(QIODevice::ReadOnly))
		{
			if (frule.read((char*)m_r2tune, sizeof(r2_tune_handle)) == sizeof(r2_tune_handle))
			{
				...
				}else ui->tabler2tune->setR2Tune(m_mode, ui->cbpath->currentIndex(), m_r2tune);
				return;
			}
		}
